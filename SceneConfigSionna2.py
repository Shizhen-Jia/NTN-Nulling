...
class SceneConfigSionna:
    def __init__(self, scene, nbs, nsect, fc = 10e9):
        ...
        self.tn_height_above_ground = 1.88
    ...
    def compute_positions(self,
        ...
        # num_ntn_building = int(0.4 * ntn_rx)
        # num_ntn_outdoor  = ntn_rx - num_ntn_building
        ...
        # inner_radius = 500.0  # 内半径，单位：米
        # outer_radius = 2500.0  # 外半径，单位：米

        # # 总共的 NT/NNT 接收机数量为 ntn_rx
        # # 生成均匀分布的随机角度 theta ∈ [0, 2π)
        # theta = 2 * np.pi * np.random.rand(ntn_rx)

        # # 为保证在面积上均匀分布，随机生成半径：
        # # 先在 [r_inner^2, r_outer^2] 上均匀采样，再取平方根
        # r = np.sqrt(np.random.uniform(inner_radius**2, outer_radius**2, ntn_rx))

        # # 将极坐标转换为笛卡尔坐标
        # rx_ntn_x = r * np.cos(theta)
        # rx_ntn_y = r * np.sin(theta)
        
        # ix = np.searchsorted(self.cm.x, rx_ntn_x)
        # iy = np.searchsorted(self.cm.y, rx_ntn_y)

        # # 注意：最好确认 ix 和 iy 都在有效索引范围内，否则需要作边界处理

        # # 根据建筑物信息确定 z 坐标
        # rx_ntn_z = np.where(
        #     self.cm.bldg_grid[iy, ix],
        #     self.cm.zmax_grid[iy, ix] + self.ntn_height_above_roof,
        #     self.cm.zmin_grid[iy, ix] + self.ntn_height_above_ground
        # )
        ...
        self.bs_tn_pos = np.array([p1,  p3, p11])
