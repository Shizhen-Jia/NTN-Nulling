...
class RadioMap(ABC):
    ...
    def inr_ntn(self):
        # pylint: disable=line-too-long
        r"""INR across the radio map from all transmitters [unitless, linear scale]

        The shape of the tensor depends on the subclass.
        Only used for NTN-Nulling project, 
        interference comes from all the base station, all signal power as interfer
        ...
        # Interference for each transmitter
        # Numerical issue can cause this value to be slightly negative
        interference = total_pow

        # Thermal noise
        noise = self._thermal_noise_power

        # SINR
        inr_ntn_map = (interference / noise)
        return inr_ntn_map
    ...
    def tx_association(self, metric: str = "path_gain") -> mi.TensorXi:
        ...
        if metric not in ["path_gain", "rss", "sinr", "inr_ntn"]:
    ...
    def sample_cells(
        ...
        # Convert to dB-scale
        if metric in ["path_gain", "sinr", "inr_ntn"]:
    ...
    def cdf(
        ...
        if metric=="path_gain":
            ...
            xlabel = "Interference-to-noise ratio (INR) [dB]"
            title = "INR"
        else:
